...
## Database choice
B5. Default per kit: Supabase Postgres. **Override for Builder (per ADR-0001)**: SQLite via better-sqlite3, accessed only by the main process. The Builder is a single-user desktop app; Postgres is wrong scope.
...
B11. MUST treat every mutation that can be retried as idempotent. The orchestrator's retries on `claude` CLI subprocess invocations MUST use a stable idempotency key (typically the task id from `build-order.md`). See ADR-0002.
...
## Auth
B13. **Override for Builder (revised per ADR-0002)**: there is no user account and no Anthropic credential held by the Builder. The `claude` CLI handles its own auth (subscription or API key configured inside the CLI). The OS keychain holds the Vercel access token used at E1. Tauri IPC commands MUST verify the calling window is the Builder's main window before executing privileged actions.
B14. **Override for Builder**: RLS does not apply (single-user, single-process). Equivalent control: the SQLite file lives in the user's home directory under OS-level permissions; only the main process opens it.
B15. (RLS performance) does not apply.
B16. (RLS index) does not apply.
B17. (RBAC vs ABAC) does not apply.
B18. **Override for Builder (revised per ADR-0002)**: the equivalent of "service-role key" is the Tauri IPC bridge. The webview MUST NOT have direct file system, network, or process-spawning access. The `claude` CLI subprocess and any other privileged operations are spawned by allow-listed Tauri commands.
...
## Background jobs
B22. **Override for Builder**: the job runner is the orchestrator, an in-process actor in the Tauri main process. Same idempotency, retry, and dead-letter principles as Inngest, but local. Inngest, QStash, Trigger.dev are NOT used.
...
B32. MUST implement a `users.delete` flow at the project level: the novice can delete a project, which removes the project folder and the `projects` row. No PII persists in the Builder DB beyond the project path.
