...
import type { RunGit, RunResult } from "../debug/repair/index.js";
...
describe("debug.applyFix handler", () => {
  it("applies an extract-secret codemod, syntax-checks, squashes, marks defect fixed", async () => {
    ...
    const result = await applyFix({ defectId }, git.asRunGit());
  ...
  it("audit emits debug_fix_applied on success", async () => {
    ...
    await applyFix({ defectId }, git.asRunGit());
  ...
  it("aborts the branch and reports skipped_no_codemod for unhandled rules", async () => {
    const projectId = await newProjectAt(projectPath);
    const defectId = await insertDefect(projectId, {
    ...
    expect(result.outcome).toBe("skipped_no_codemod");
  ...
  it("rejects an unknown defectId", async () => {
    await expect(applyFix({ defectId: "does-not-exist" })).rejects.toThrow(
      /defect not found/
    );
  });
